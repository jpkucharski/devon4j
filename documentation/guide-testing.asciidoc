...
public class StaffmanagementImplTest extends ModuleTest {
  ...
  @InjectMocks
	private StaffmanagementImpl staffMangementImpl = new StaffmanagementImp();
  @Mock 
...
I JUnit5 there was introduced @BeforeEach and @AfterEach annotation that is allowing to create a rule that will be invoked before and after each Test.
Static imports are a convenient option to enhance readability within Mockito tests.
